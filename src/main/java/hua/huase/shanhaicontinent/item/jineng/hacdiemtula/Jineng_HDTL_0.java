...
public class Jineng_HDTL_0 extends WuqiBase {
    public Jineng_HDTL_0(Tier tier, int i, float v, Properties properties) {
    ...
    public float getWugong(Player player, ItemStack itemStack, float value, EquipmentSlot offhand) {
        if(offhand == EquipmentSlot.MAINHAND){
            ...
            value += nianxian*0.15f*(dengji+1)/10 ;
        }
    ...
    public void appendHoverText(ItemStack itemStack, @Nullable Level level, List<Component> list, TooltipFlag tooltipFlag) {
        ...
        list.add(Component.literal("Sát thương của vũ khí dựa và số năm hồn hoàn đầu tiên").withStyle(ChatFormatting.RED));
        list.add(Component.literal("Mỗi 10 cấp tăng 15% sát thương, 10% tỷ lệ chí mạng và 25% damage chí mạng").withStyle(ChatFormatting.GREEN));
    }
}
