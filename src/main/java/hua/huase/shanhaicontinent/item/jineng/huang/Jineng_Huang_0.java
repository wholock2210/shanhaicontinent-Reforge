...
public class Jineng_Huang_0 extends WuqiBase {
    ...
    public float getZhenshang(Player player, ItemStack itemStack, float value, EquipmentSlot offhand) {
        if(offhand == EquipmentSlot.OFFHAND){
            float dengji = PlayerAttrubuteAPI.getDengji(player);
            value += value*0.04f*(dengji+1)/10;
        }
    ...
    public float getKangbao(Player player, ItemStack itemStack, float value, EquipmentSlot offhand) {
        if(offhand == EquipmentSlot.OFFHAND){
            float dengji = PlayerAttrubuteAPI.getDengji(player);
            value += 4f*(dengji+1)/10;
        }
    ...
    public void appendHoverText(ItemStack itemStack, @Nullable Level level, List<Component> list, TooltipFlag tooltipFlag) {
        ...
        list.add(Component.translatable("每10级提升4%的物攻、物防、真伤、爆伤、抗暴、回复").withStyle(ChatFormatting.GREEN));
        list.add(Component.translatable("--副手生效").withStyle(ChatFormatting.DARK_GRAY));
    }
}
