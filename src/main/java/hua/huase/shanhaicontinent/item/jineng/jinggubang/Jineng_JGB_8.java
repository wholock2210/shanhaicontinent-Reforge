...
public class Jineng_JGB_8 extends JinengBase{
    public Jineng_JGB_8(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        player.getCooldowns().addCooldown(this, (int) (1200-Math.log10(this.getNianxian(player, itemstack))*100));
        if (!level.isClientSide) {
            List<LivingEntity> nearbyEntities = level.getEntities(EntityTypeTest.forClass(LivingEntity.class), player.getBoundingBox().inflate(20,6,20), Entity::isAlive);
            for (LivingEntity nearbyEntity : nearbyEntities) {
                if(nearbyEntity.getId() != player.getId()){

                    nearbyEntity.hurt(level.damageSources().playerAttack(player), AttrubuteAPI.getWugong((player))*3.0f);
                }else {
                    player.addEffect(new MobEffectInstance(SHModMobEffectsinit.jineng_jgb_8.get(), 600, 2));
                }
            }
        }
...
//        }
    ...
    public void appendHoverText(ItemStack itemStack, @Nullable Level level, List<Component> list, TooltipFlag tooltipFlag) {
        ...
        list.add(Component.translatable("上古神王附体").withStyle(ChatFormatting.GREEN));
