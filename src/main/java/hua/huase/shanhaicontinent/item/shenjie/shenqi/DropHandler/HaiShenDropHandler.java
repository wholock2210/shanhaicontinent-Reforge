...
import hua.huase.shanhaicontinent.item.shenjie.HaiShen;
...
public class HaiShenDropHandler {
    ...
    public static void onLivingDeath(LivingDeathEvent event) {
        ...
        if (event.getSource().getEntity() instanceof Player player) {
            ...
            if (offhandItem.is(ModItems.LINGPAI_HAISHEN.get())) {
                LivingEntity killedEntity = event.getEntity();
                float dropChance = getDropChanceForEntity(killedEntity);

                if (player.getRandom().nextFloat() < dropChance) {
                    ItemStack fragment = new ItemStack(ModItems.haiyang_suipian.get());
                    // 修正：直接生成掉落物（会自动合并）
