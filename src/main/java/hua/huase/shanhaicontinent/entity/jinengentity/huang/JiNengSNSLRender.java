...
public class JiNengSNSLRender extends EntityRenderer<JiNengSNSLEntity> {

    public JiNengSNSLRender(EntityRendererProvider.Context context) {
        super(context);
    }

    public void render(JiNengSNSLEntity p_115266_, float p_115267_, float p_115268_, PoseStack p_115269_, MultiBufferSource p_115270_, int p_115271_) {
        ...
        for(int i = 7; i >= 0; --i) {
            afloat[i] = f;
            afloat1[i] = f1;
            f += (float)(randomsource.nextInt(4) -2);
            f1 += (float)(randomsource.nextInt(4) - 2);
        }
        ...
        for(int j = 0; j < 4; ++j) {
            RandomSource randomsource1 = RandomSource.create(random.nextLong());
            ...
            for(int k = 0; k < 3; ++k) {
                ...
                for(int j1 = l; j1 >= i1; --j1) {
                    ...
                    float f5 = f3;
                    if (k == 0) {
                        f2 += (float)(randomsource1.nextInt(4) - 2);
                        f3 += (float)(randomsource1.nextInt(4) - 2);
                    } else {
                        f2 += (float)(randomsource1.nextInt(4) - 2);
                        f3 += (float)(randomsource1.nextInt(4) - 2);
                    }
    ...
    @Override
    public ResourceLocation getTextureLocation(JiNengSNSLEntity p_115264_) {
